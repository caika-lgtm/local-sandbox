...
- Tests run: `cargo fmt --all -- --check`; `cargo check --workspace`; `cargo test -p lsb-platform windows_x86_64::fs -- --nocapture`; `cargo test -p lsb-vm -- --nocapture`; `cargo test -p lsb-guest -- --nocapture`; targeted Windows test compiles for `lsb-platform`/`lsb-vm`; `cargo test --workspace`; `./scripts/win-gh-test unit` runs `28714105683`, `28729436438`, and `28730489407`; `./scripts/win-gh-test smoke` runs `28714141992` and `28730520707`.
- Debug artifacts: Windows smoke run `28714141992`, job `85153326347`, uploaded `windows-lsb-diagnostics` artifact ID `8084000670`; review-fix Windows unit run `28729436438`, job `85192454081`, uploaded artifact ID `8088274579`; review-fix Windows smoke run `28730520707`, job `85195259951`, uploaded artifact ID `8088597449`.
